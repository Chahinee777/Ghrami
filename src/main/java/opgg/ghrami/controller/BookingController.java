...
public class BookingController {
    ...
    /**
     * Get the active (non-cancelled) booking for a user + class, or null if none.
     */
    public Booking getUserBookingForClass(Long userId, Long classId) {
        String sql = "SELECT b.*, " +
                "  c.title as class_title, c.category as class_category, c.duration as class_duration, c.provider_id, " +
                "  u.username, u.full_name as user_full_name, u.email as user_email, " +
                "  p_user.full_name as provider_name " +
        ...
        try (Connection conn = DatabaseConnection.getInstance().getConnection();
            ...
            System.err.println("Error fetching booking for class: " + e.getMessage());
        }
        return null;
    }
    ...
    private Booking mapResultSetToBooking(ResultSet rs) throws SQLException {
        ...
        // Rating / review / watch progress (new columns — safe fallback)
        try {
            int ratingVal = rs.getInt("rating");
            if (!rs.wasNull()) booking.setRating(ratingVal);
        } catch (SQLException ignored) {}
        try { booking.setReview(rs.getString("review")); } catch (SQLException ignored) {}
        try { booking.setWatchProgress(rs.getInt("watch_progress")); } catch (SQLException ignored) {}

        return booking;
    }

    /**
     * Persist the student's last watched position for a booking.
     * Called every 15 seconds and on video window close.
     */
    public boolean updateWatchProgress(Long bookingId, int seconds) {
        String sql = "UPDATE bookings SET watch_progress = ? WHERE booking_id = ?";
        try (Connection conn = DatabaseConnection.getInstance().getConnection();
            ...
            System.err.println("updateWatchProgress error: " + e.getMessage());
            return false;
        }
    }

    /**
     * Save a student's star rating and optional review text.
     * Called from ClassMarketplaceController.showRatingDialog().
     */
    public boolean updateRatingAndReview(Long bookingId, int rating, String review) {
        String sql = "UPDATE bookings SET rating = ?, review = ? WHERE booking_id = ?";
        try (Connection conn = DatabaseConnection.getInstance().getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, rating);
            if (review != null) ps.setString(2, review); else ps.setNull(2, java.sql.Types.VARCHAR);
            ps.setLong(3, bookingId);
            return ps.executeUpdate() > 0;
        } catch (SQLException e) {
            System.err.println("updateRatingAndReview error: " + e.getMessage());
            return false;
        }
    }
